...
public interface ChatRoomRepository extends JpaRepository<ChatRoom, Long> {
    ...
    @Query("select cm, cp, (select count(cm2) from ChatMessage cm2 where cm2.chatRoom = cr and cm2.chatMessageRead = false and cm.chatMessageSenderId <> :memberId) from ChatRoom cr left join ChatParticipants cp on cr.chatRoomId = cp.chatRoom.chatRoomId left join ChatMessage cm on cr.chatRoomId = cm.chatRoom.chatRoomId "
